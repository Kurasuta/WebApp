...
function forwardToRandomSampleIn(year) {
    new KurasutaApi().randomSampleByYear(year).then(response => {
...
export default function BuildTimeStampChart() {
    const [buildTimestampsByYear, setBuildTimestampsByYear] = useState(null);
    useEffect(() => {
        new KurasutaApi().buildTimeStampsByYear().then((result) => {
            setBuildTimestampsByYear(result.data)
        });
    }, []);
    ...
    if (buildTimestampsByYear === null) {
        ...
        buildTimeStampChart = <C3Chart
            data={{
                ...
                onclick: d => {
                    setBuildTimestampsByYear(null);
                    forwardToRandomSampleIn(d.x);
                },
                type: 'bar'
            }}
    ...
    return <div>
        ...
        {buildTimeStampChart}
        <p>
            The above diagram shows a histogram of build time stamps by year of all samples we processed so far. It
            will
            most probably show a spike in 1970 and 1992. 1970 is the beginning of the UNIX epoch and the spike
            simple
            means that we could not extract a build time stamp from those samples. The spike in 1992 can be
            explained by
        ...
        <p>
            The early decline from 2012 on is caused by the fact that we are still missing fresh sample sources. If
            you
